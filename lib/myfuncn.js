...
exports.runtime = function(seconds) {
	seconds = Number(seconds);	var days = Math.floor(seconds / (3600 * 24));
	var hours = Math.floor(seconds % (3600 * 24) / 3600);
	var minutes = Math.floor(seconds % 3600 / 60);
	var sec = Math.floor(seconds % 60);
	var daysDisplay = days > 0 ? days + (days == 1 ? " days, " : " days, ") : "";
	var hoursDisplay = hours > 0 ? hours + (hours == 1 ? " hours, " : " hours, ") : "";
	var minutesDisplay = minutes > 0 ? minutes + (minutes == 1 ? " minutes, " : " minutes, ") : "";
	var secDisplay = sec > 0 ? sec + (sec == 1 ? " sec" : " sec") : "";
	return daysDisplay + hoursDisplay + minutesDisplay + secDisplay;
}
exports.clockString = function(seconds) {
    let hours = isNaN(seconds) ? '--' : Math.floor(seconds % (3600 * 24) / 3600)
    let minutes = isNaN(seconds) ? '--' : Math.floor(seconds % 3600 / 60)
    let sec = isNaN(seconds) ? '--' : Math.floor(seconds % 60)
    return [hours, minutes, sec].map(v => v.toString().padStart(2, 0)).join(':')
}
...
exports.formatDate = (n, locale = 'id') => {
	let days = new Date(n)
	return days.toLocaleDateString(locale, {
		weekday: 'long',
		day: 'numeric',
		month: 'long',
		year: 'numeric',
